...
from app.modules.gmail_sync.schemas import GmailMessage, SyncResult, GmailSyncConfig
...
def create_auth_flow(redirect_uri: Optional[str] = None) -> Flow:
    ...
    client_config = {
        "web": {
            ...
            "redirect_uris": [redirect_uri or os.getenv("GMAIL_REDIRECT_URI", "http://localhost:8000/api/v1/gmail/callback")]
        }
    }
    ...
    flow = Flow.from_client_config(
        client_config,
        scopes=SCOPES,
        redirect_uri=redirect_uri or os.getenv("GMAIL_REDIRECT_URI", "http://localhost:8000/api/v1/gmail/callback")
    )
    return flow
