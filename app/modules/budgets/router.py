...
def create(payload: BudgetCreate, db: Session = Depends(get_db)):
    user_id = 1
    return create_budget(db, user_id=user_id, payload=payload)
...
def summary(budget_id: int, include_subcategories: bool = True, db: Session = Depends(get_db)):
    user_id = 1
